...
const CourseDetailPage = ({
  ...
  const decodedSlug = courseSlug
    ? decodeURIComponent(courseSlug).toLowerCase().trim()
    : "";
...
const matchedCourses = useMemo(() => {
  ...
  // 🔍 DEBUG LOG
  console.log("🔍 Searching for course:", decodedSlug);

  colleges.forEach((college: any) => {
    const courseArr = college.rawScraped?.courses;
    if (!Array.isArray(courseArr)) return;

    courseArr.forEach((course: any) => {
      const nameKey = course.name?.toLowerCase().trim();
      
      // 🔍 DEBUG: Log all course names
      console.log(`College: ${college.name}, Course: ${course.name} → ${nameKey}`);
      
      if (nameKey === decodedSlug) {
